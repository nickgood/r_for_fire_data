...
* To create am **R Studio** project navigate to: `File -> New Project -> Create Project` (giving the new folder a name via the dialog).
...
Some additional tidyverse packages we'll use are `magrittr` that adds operators to make code more readible and `lubridate` for date-time manipulation.
...
The `tidyverse` includes the powerfull `ggplot2` library. For spatial analysis we'll include install the `ggmaps` and `plotly` packages. We'll also install the `ggthemes` package.
...
You can write code that is evaluated inline with your text using `r sourcecode`. For example we can count the number flights in the dataset using `r sourcecode2`. So that we print: ***There are `r length(unique(arctas_data$flight))` flights in the dataset***.

* Add a sentence to your markdown document that includes the number of flights in the dataset.
...
Or you can include them explicitely:
...
* Data of class `tbl` display in a convient manner when the object is passed to the console
...
Inevitably you will encounter data in incinvient layouts. The `tidyr` package contains tools for reshaping your data.
...
Often datasets need to be combined. The `dplyr` package contains various functions function types for combining data.
...
The `group_by()` and `summarize` functions are useful for data reduction. Often you'll want to calculate statisitics by groups within your data

* The `group_by()` function groups data by one or more variables. The data will look the same, but the results of subcequent operations on the data will be applied by groups.

* We can use `summarize()` to calculate statisitics by group for a given column (variable)

* We can use `summarize_all()` to calculate the statisitic for all the columns in a data frame.

* The data can be ungrouped using the `ungroup()` function.

```
 * Group the data by flight and caluculate the mean and standard deviation of the ozone mixing ratio
```
