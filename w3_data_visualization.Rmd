...
```{r load_data}
 ...
 # grab the first flight
  data_first_flight <- filter(data_arctas, flight == first(unique(flight)))
```
...
```{r}
  ggplot(data = data_first_flight, mapping = aes(x = utc, y = o3)) +
  geom_point()
```
...
```{r}
  p <- ggplot(data = data_first_flight, mapping = aes(x = utc, y = o3)) +
       geom_point()
  p
```
...
```{r}
  ...
  ggplot(p_data, aes(flight, mean_no)) +
  geom_point()
```
...
```{r}
  ...
  ggplot(p_data, aes(rank, mean_no)) +
  geom_point()
```
...
```{r}
  p <- ggplot(p_data, aes(rank, mean_no)) +
       geom_point() +
       geom_smooth() +
       geom_text(aes(label = flight), hjust = 0.5, vjust = -0.5)
  p
```
