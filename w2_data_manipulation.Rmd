...
There are a number of `dplyr` functions for merging data. Two common tasks are combining rows of data with the same columns and adding columns to a dataset.
...
* You can use the `_join()` functions to merge datasets by a common column or columns:
...
You can use the `%>%` operator from the `magrittr` package to make your code more readible:
...
1. Load the  `O3`, `NO`, `NO2`, and `NOY` data from the `flt20130603` dataset into separate objects.
